...
for (var i = 0; i < ds_list_size(_targets); i++) {
	...
	if (frictionless and _targeted.object_index == obj_player) {
		_targeted.xVelocityFrictionless = x_vel + (_targeted.xVelocityFrictionless * additive)
		_targeted.alarm[playerAlarms.removeFrictionlessXVelocity] = frictionlessTimeout
	}
	if (x_vel != 0) {
		_targeted.xVelocity = x_vel + (_targeted.xVelocity * additive)
	}
	if (y_vel != 0) {
		_targeted.grav = y_vel + (_targeted.grav * additive)
